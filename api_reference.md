Here's the new documentation page content for the SpriteAI API reference:

<response>
# SpriteAI API Reference

This document provides a comprehensive API reference for the SpriteAI library, detailing all public functions, their parameters, return values, and usage examples.
...
```javascript
const sprite = await generateCharacterSpritesheet('a heroic knight in shining armor', {
...
- `options` (object, optional): Configuration options for the environment sprite generation.
  ...
  - `padding` (number, default: 1): Padding between sprites in the tileset.
...
```javascript
const environment = await generateEnvironmentSprites('a mystical forest with ancient ruins', {
  elements: 6,
  size: '2048x2048',
  style: 'pixel-art',
  theme: 'fantasy',
...
This API reference provides a comprehensive guide to the main functions available in the SpriteAI library. Each function is documented with its syntax, parameters, return values, and example usage to help developers integrate and utilize the library effectively in their projects.
